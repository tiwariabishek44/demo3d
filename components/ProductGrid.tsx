        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 auto-rows-[minmax(450px,auto)]">
          {PRODUCTS.map((product) => (
            <motion.div
              key={product.id}
              variants={itemVariants}
              className={`group relative flex flex-col overflow-hidden rounded-[3rem] bg-[#F5F5F7] transition-all duration-1000 ease-[0.22,1,0.36,1] hover:shadow-[0_60px_100px_rgba(0,0,0,0.08)] hover:-translate-y-2 ${product.featured ? "md:col-span-2 md:row-span-2" : "col-span-1"
                }`}
            >
              ...
              <div className="p-12 pb-0 z-10 flex justify-between items-start">
                ...
                <div className="flex flex-col items-end gap-4">
                  ...
                  <button className="px-8 py-3 rounded-full bg-[#1D1D1F] text-white text-[11px] font-black uppercase tracking-widest transition-all duration-500 hover:bg-[#0050FF] hover:scale-110 active:scale-95 shadow-xl shadow-black/5">
                    Buy
                  </button>
                </div>
              </div>
